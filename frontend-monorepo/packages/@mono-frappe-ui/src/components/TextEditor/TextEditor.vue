<template>
	<div v-if="editor" class="relative w-full" :class="attrsClass" :style="attrsStyle">
...
onMounted(() => {
	const defaultExtensionsDict = {
		...
		"textStyle": TextStyle,
		"namedColorExtension": NamedColorExtension,
		"namedHighlightExtension": NamedHighlightExtension,
		"codeBlockLowlight": CodeBlockLowlight.extend({
			addNodeView() {
				return VueNodeViewRenderer(CodeBlockComponent)
			},
		}).configure({ lowlight }),
		"imageExtension": ImageExtension.configure({
			uploadFunction: props.uploadFunction || defaultUploadFunction,
		}),
		"imageGroup": ImageGroup.configure({
			uploadFunction: props.uploadFunction || defaultUploadFunction,
		}),
		"imageViewerExtension": ImageViewerExtension,
		"videoExtension": VideoExtension.configure({
			uploadFunction: props.uploadFunction || defaultUploadFunction,
		}),
		"linkExtension": LinkExtension.configure({
			openOnClick: false,
		}),
		...
		"mention": configureMention(props.mentions),
		"emojiExtension": EmojiExtension,
		"slashCommands": SlashCommands,
		"tagNode": TagNode,
		"tagExtension": TagExtension.configure({
			tags: () => props.tags,
		}),
		"markdownPasteExtension": MarkdownPasteExtension.configure({
			enabled: true,
			showConfirmation: true,
		}),
		...(props.extensions || {})
	}
	const extensions = Object.values(defaultExtensionsDict)
