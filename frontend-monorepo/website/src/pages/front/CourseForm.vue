    ...
    <div class="grid md:grid-cols-[70%,30%] h-full">
      <div>
        <header
          ...
          <div class="flex items-center mt-3 md:mt-0 gap-3">
            <Button v-if="courseResource.data?.name" @click="trashCourse()">
              <template #icon>
                <Trash2 class="w-4 h-4 stroke-1.5" />
              </template>
            </Button>
            <Button variant="solid" @click="submitCourse()" class="xl py-4 px-5 text-lg font-semibold rounded-lg"
        ...
        <div class="mt-5 mb-5">
          <div class="px-10 pb-5 mb-5 space-y-5 border-b">
            ...
            <div class="grid grid-cols-2 gap-5">
              <FormControl v-model="course.title" :label="__('Title')" :required="true" />
            ...
            <div class="grid grid-cols-2 gap-5">
              <MultiSelect v-model="instructors" doctype="User" :label="__('Instructors')"
                :filters="{ ignore_user_type: 1 }" :onCreate="(close) => openSettings('Members', close)"
                :required="true" />
              <div>
                ...
                <div class="flex items-center">
                  ...
                  <FormControl v-model="newTag" :placeholder="__('Add a keyword and then press enter')" class="w-full"
                    @keyup.enter="updateTags()" id="tags" />
                </div>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-5">
              <FormControl v-model="course.short_introduction" type="textarea"
                class="!border-[#f1511b] !focus:border-[#f1511b] !focus:ring-[#f1511b]" variant="text-ink-[#f1511b]"
                :rows="5" :label="__('Short Introduction')" :placeholder="__(
              ...
              <div class="mb-4">
                ...
                <FileUploader v-if="!course.course_image" :fileTypes="['image/*']" :validateFile="validateFile"
                  ...
                  <template v-slot="{ file, progress, uploading, openFileSelector }">
                    <div class="flex items-center">
                      ...
                      <div class="ml-4">
                        <Button class="xl py-4 px-5 text-lg font-semibold rounded-lg text-white"
                          @click="openFileSelector" style="background-color: rgb(241, 81, 27)">
                          {{ __('Upload') }}
                        </Button>
                ...
                <div v-else class="mb-4">
                  <div class="flex items-center">
                    ...
                    <div class="ml-4">
                      <Button class="xl py-4 px-5 text-lg font-semibold rounded-lg text-white" @click="removeImage()"
                        style="background-color: rgb(241, 81, 27)">
                        {{ __('Remove') }}
                      </Button>
          ...
          <div class="px-10 pb-5 mb-5 space-y-5 border-b">
            <div class="">
              ...
              <TextEditor :content="course.description" @change="(val) => (course.description = val)" :editable="true"
                :fixedMenu="true"
            ...
            <MultiSelect v-model="related_courses" doctype="LMS Course" :label="__('Related Courses')"
              :filters="{ name: ['!=', courseResource.data?.name] }" :onCreate="(close) => {
                  router.push({
                    name: 'CourseForm',
          ...
          <div class="px-10 pb-5 space-y-5 border-b">
            ...
            <div class="grid grid-cols-3">
              ...
              <FormControl type="checkbox" v-model="course.paid_certificate" :label="__('Paid Certificate')" />
            </div>
            <div class="grid grid-cols-2 gap-5">
              <div class="space-y-5">
                <FormControl v-if="course.paid_course || course.paid_certificate" v-model="course.course_price"
                  :label="__('Amount')" />
                <Link v-if="course.paid_certificate" doctype="Course Evaluator" v-model="course.evaluator"
                  :label="__('Evaluator')" :onCreate="(value, close) => openSettings('Evaluators', close)
                    " />
              </div>
              <Link v-if="course.paid_course || course.paid_certificate" doctype="Currency" v-model="course.currency"
                :filters="{ enabled: 1 }" :label="__('Currency')" />
            </div>
          </div>
      ...
      <div class="border-l">
        <CourseOutline v-if="courseResource.data" :courseName="courseResource.data.name" :title="__('Course Outline')"
          :allowEdit="true" />
      </div>
    </div>
  </div>
