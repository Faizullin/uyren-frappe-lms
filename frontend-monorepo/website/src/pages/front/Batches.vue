<template>
  <header
    ...
    <router-link v-if="canCreateBatch()" :to="{
      name: 'BatchForm',
      params: { batchName: 'new' },
    }">
      <Button class="xl py-4 px-5 text-lg font-semibold rounded-lg" variant="solid"
  ...
  <div class="p-5 pb-10">
    <div class="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:items-center justify-between mb-5">
      <div v-if="!isAdmin" class="hidden xl:block text-xl text-ink-gray-9 font-semibold">
        {{ __('All Batches') }}
      </div>

      <div class="flex flex-col xl:flex-row space-y-4 xl:space-y-0 xl:items-center justify-between mb-5">
        <CustomTabButtons :buttons="batchTabs" v-model="currentTab" />

        <div class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-4 md:space-y-0">
          <FormControl v-model="certification" :label="__('Certification')" type="checkbox" size="2xl"
            class="p-[10px] text-[18px]" @change="updateBatches()" />

          <FormControl v-model="title" :placeholder="__('Search by Title')" type="text" size="xl" variant="outline"
            class="md:w-[200px] lg:w-[200px] xl:w-[200px] max-w-full text-[#f1511b]" @input="updateBatches()" />

          <div class="min-w-40 lg:min-w-0 lg:w-32 xl:w-40">
            <Select v-if="categories.length" v-model="currentCategory" :options="categories"
              :placeholder="__('Category')" size="xl" variant="outline" @change="updateBatches()" />
          </div>
        </div>
      </div>
    </div>
...
const isAdmin = computed(() =>
  user?.data?.is_system_manager || user?.data?.role === 'Administrator'
)
