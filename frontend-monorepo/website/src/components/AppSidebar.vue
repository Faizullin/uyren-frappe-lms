<template>
  <div class="flex h-full flex-col justify-between transition-all duration-300 ease-in-out border-r bg-surface-menu-bar"
    ...
    <div class="flex flex-col overflow-hidden" :class="sidebarStore.isSidebarCollapsed ? 'items-center' : ''">
      ...
      <div class="flex flex-col" v-if="sidebarSettings.data">
        <SidebarLink v-for="link in sidebarLinks" :link="link" :isCollapsed="sidebarStore.isSidebarCollapsed"
          class="mx-2 my-0.5 py-4" />
      </div>
      <!-- <div
...
<div v-if="sidebarSettings.data?.web_pages?.length" class="flex flex-col transition-all duration-300 ease-in-out"
  ...
  <SidebarLink v-for="link in sidebarSettings.data.web_pages" :link="link"
    :isCollapsed="sidebarStore.isSidebarCollapsed" class="mx-2 my-0.5" :showControls="isModerator ? true : false"
    @openModal="openPageModal" @deletePage="deletePage" />
</div>
</div> -->
    </div>
    <div class="m-2 flex flex-col gap-1">
      <div>
        ...
        <div class="flex justify-center gap-4">
          <a href="https://www.instagram.com/uyrenai?igsh=bXZhN3gzeHQxc3lq" class="social-icon"><img
              src="/img/icons8-instagram-32.png" alt="Instagram" width="37" height="37" /></a>
          <a href="https://t.me/uyrenai" class="social-icon"><img src="/img/icons8-telegram-48.png" alt="Telegram"
              width="38" height="38" /></a>
          <a href="https://wa.me/77780080502" class="social-icon"><img src="/img/icons8-whatsapp-logo-32.png"
              alt="WhatsApp" width="35" height="35" /></a>
          <a href="https://www.linkedin.com/company/uyrenai/" class="social-icon"><img
              src="/img/icons8-linkedin-logo-32.png" alt="LinkedIn" width="37" height="37" /></a>
          <a href="https://discord.gg/x3SWq3pU"><img src="/img/discord-outline.svg" alt="discord" class="w-8 h-8" /></a>
        </div>
        <p class="text-center text-xs text-gray-500 mt-4">© 2025 Uyren.Ai. {{ __('All rights reserved.') }}</p>
      </div>
      <div v-if="readOnlyMode && !sidebarStore.isSidebarCollapsed"
        class="z-10 m-2 bg-surface-modal py-2.5 px-3 text-xs text-ink-gray-7 leading-5 rounded-md">
        {{ __('This site is being updated. You will not be able to make any changes. Full access will be restored shortly.')}}
      </div>
      <!-- <TrialBanner
				v-if="
      ...
      <div class="flex items-center mt-4" :class="sidebarStore.isSidebarCollapsed ? 'flex-col space-y-3' : 'flex-row'
        ">
        <div class="flex items-center flex-1" :class="sidebarStore.isSidebarCollapsed
          ? 'flex-col space-y-3'
          : 'flex-row space-x-3'
          ">
          ...
          <Tooltip v-if="showOnboarding" :text="__('Help')">
            <CircleHelp class="size-4 stroke-1.5 text-ink-gray-7 cursor-pointer" @click="
              () => {
        ...
        <Tooltip :text="sidebarStore.isSidebarCollapsed ? __('Expand') : __('Collapse')
          ">
          <CollapseSidebar class="size-4 text-ink-gray-7 duration-300 stroke-1.5 ease-in-out cursor-pointer" :class="{
            '[transform:rotateY(180deg)]': sidebarStore.isSidebarCollapsed,
          }" @click="toggleSidebar()" />
        </Tooltip>
      </div>
    </div>
    <HelpModal v-if="showOnboarding && showHelpModal" v-model="showHelpModal" v-model:articles="articles"
      appName="learning" title="Frappe Learning" :logo="LMSLogo"
      :afterSkip="(step) => capture('onboarding_step_skipped_' + step)"
      :afterSkipAll="() => capture('onboarding_steps_skipped')"
      :afterReset="(step) => capture('onboarding_step_reset_' + step)"
      :afterResetAll="() => capture('onboarding_steps_reset')" docsLink="https://docs.frappe.io/learning" />
