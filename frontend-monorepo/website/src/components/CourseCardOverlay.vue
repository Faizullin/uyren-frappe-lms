  ...
  <div class="border-2 rounded-md min-w-80">
    ...
    <div class="p-5">
      ...
      <div v-if="!readOnlyMode">
        ...
        <Button v-else @click="enrollStudent()" variant="solid" class="w-full button" size="md">
          <span>
            {{ __('Start Learning') }}
          </span>
        </Button>
        ...
        <router-link v-if="user?.data?.is_moderator || is_instructor()" :to="{
          ...
          <Button variant="subtle" class="w-full mt-2 text-white" size="md" style="background-color: rgb(241, 81, 27)">
            <span>
              {{ __('Edit') }}
            </span>
          </Button>
      ...
      <div class="space-y-4">
        ...
        <div v-if="course.data.enable_certification" class="flex items-center font-semibold text-ink-gray-9">
          <GraduationCap class="h-4 w-4 stroke-2" />
          <span class="ml-2">
            {{ __('Certificate of Completion') }}
          </span>
        </div>
        <div v-if="course.data.paid_certificate" class="flex items-center font-semibold text-ink-gray-9">
          <GraduationCap class="h-4 w-4 stroke-2" />
          <span class="ml-2">
