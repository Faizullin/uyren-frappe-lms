<template>
  <div class="">
    <div v-if="title && (outline.data?.length || allowEdit)"
      ...
      <div class="font-semibold text-lg leading-5 text-ink-gray-9" :class="{ 'font-medium text-p-base': allowEdit }">
        {{ __(title) }}
      </div>
      <Button size="md" class="text-white" v-if="allowEdit" @click="openChapterModal()"
          ...
          <div class="flex ml-auto space-x-4">
            <Tooltip :text="__('Edit Chapter')" placement="bottom">
              <FilePenLine v-if="allowEdit" @click.prevent="openChapterModal(chapter)"
                class="h-6 w-6 text-ink-gray-9 invisible group-hover:visible" />
            </Tooltip>
            <Tooltip :text="__('Delete Chapter')" placement="bottom">
              <Trash2 v-if="allowEdit" @click.prevent="trashChapter(chapter.name)"
                class="h-6 w-6 text-ink-red-3 invisible group-hover:visible" />
            </Tooltip>
          </div>
        </DisclosureButton>
        <DisclosurePanel v-if="!chapter.is_scorm_package">
              ...
              <div class="outline-lesson pl-8 py-2 pr-4 text-ink-gray-9" :class="isActiveLesson(lesson.number) ? 'bg-surface-gray-3' : ''
                ...
                <router-link :to="{
                  ...
                  <div class="flex items-center text-sm leading-5 group">
                    <MonitorPlay v-if="lesson.icon === 'icon-youtube'" class="h-5 w-5 text-[#f1511b] stroke-1 mr-2" />
                    <HelpCircle v-else-if="lesson.icon === 'icon-quiz'" class="h-5 w-5 text-[#f1511b] stroke-1 mr-2" />
                    <FileText v-else-if="lesson.icon === 'icon-list'" class="h-5 w-5 text-[#f1511b] stroke-1 mr-2" />
                    {{ lesson.title }}
                    <Trash2 v-if="allowEdit" @click.prevent="trashLesson(lesson.name, chapter.name)"
                      class="h-5 w-5 text-ink-red-3 ml-auto invisible group-hover:visible" />
          ...
          <div v-if="allowEdit" class="flex mt-2 mb-4 pl-8">
            <router-link v-if="!chapter.is_scorm_package" :to="{
              ...
              <Button size="md" class="text-white" style="background-color: rgb(241, 81, 27)">
                {{ __('Add Lesson') }}
              </Button>
            </router-link>
          </div>
